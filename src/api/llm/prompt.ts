...
5. 若用户说"没有不适""挺好的""没什么不舒服""健康""还好"等 → intent为"none"、sub_type为"other"
6. 若用户说"我想调理身体""想养生""想改善体质"但没提具体症状 → intent为"chronic"、symptom为null
7. 若完全无法识别 → intent为null
...
泛化理解是第一优先级：用户说法不必精确匹配选项标签。任何语义相近的表达都应映射到最接近的选项，而非因为没有精确匹配就放弃。

具体规则：
1. 任何表达"冷/凉/畏寒"语义的都归入怕冷类，任何表达"热/烫/发热"语义的都归入发热类
2. 程度修饰词（有点/微微/稍微/比较/很/特别）不影响分类方向——"有点发热"和"发热"归入同一选项
3. 选项后的"(含义：...)"是语义描述，用于帮助理解该选项涵盖的语义范围，据此泛化匹配用户回答
...
## 模糊匹配规则（最高优先级！高于其他所有规则）

当用户的回答只表达了上位症状概念，但没有指定区分维度时，**绝对不要**强行选择一个选项，必须返回matchedLabels列出所有相关候选：
- 用户说"口渴"/"经常口渴"/"老是口渴"/"嘴巴干" → 没有说喜喝热还是凉 → matchedLabels=["口渴喜热饮","口渴喜凉饮","口干但不想喝水"], clarificationQuestion="请问您口渴是想喝热的还是凉的？还是口干但不想喝水？"
- 用户说"怕冷"/"觉得冷"/"有点冷" → 没有说是整体还是局部 → matchedLabels=所有含"怕冷"的选项, clarificationQuestion="请问您怕冷是整体怕冷还是某个具体部位怕冷？"
- 用户说"出汗"/"老出汗" → 没有说是自汗还是盗汗 → matchedLabels=所有含"出汗/汗"的选项, clarificationQuestion="请问您出汗是白天清醒时出还是睡觉时出？"
- 用户说"心烦"/"烦躁" → 没有说是白天还是夜间 → matchedLabels=所有含"心烦"的选项, clarificationQuestion="请问您心烦是白天还是夜间？"
- **追问语必须使用选项的含义描述(semanticDesc)而非选项标签名(label)来提问**，确保用户二次回答时能从语义层面匹配到正确选项。例如选项label是"口干但不想喝水"、含义是"嘴巴干但不想喝水"，追问时应说"口干但不想喝水"而非自行改写为"口渴不想喝水"
- 当用户回答足够明确能精确匹配一个选项时（如"口渴想喝热的"明确指向"口渴喜热饮"） → matchedLabel设为该选项，matchedLabels为空数组，clarificationQuestion为null
...
5. **模糊优先原则**：当用户的回答无法精确区分多个相关选项时，不要强行选一个，必须返回matchedLabels列出所有候选 + clarificationQuestion追问区分维度。只有用户明确指定了区分维度时才用matchedLabel精确匹配
6. 用户的描述可能包含额外信息（程度、时间等），只要核心症状能映射即可
